...
function getRealIpAddr()
{
    ...
    if(isset($username) && isset($password) && isset($attendanceData) && isset($settingsData))
    {
        ...
        if(mysql_num_rows($result) == 1)        
        {
            ...
            {
                $addQuery = "UPDATE userInfo SET IP_lastAccess = '" . getRealIpAddr() . "' , attendanceData = '" . $attendanceData ."' , settingsData = '" . $settingsData . "' WHERE username LIKE '" . $username . "'";
                $addResult = mysql_query($addQuery) or die("update error");
            }
            else
            {
